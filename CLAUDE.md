...
## Table Selection Enrichment

`tryEnrichTableFragment(range)` в `src/browser.ts` — хелпер для умного захвата выделений из таблиц:

- `cloneContents()` на `<tr>` — возвращает фрагмент без обёртки `<table>`, rules не распознают таблицу
- **Решение:** перед `cloneContents()` находим родительский `<table>`, берём `<thead tr>` и строим полноценный фрагмент
- Дедупликация: если шапка уже вошла в выделение (сравниваем `textContent`) — не дублируем
- Тесты идут через `selectionPipeline()` (имитация pipeline); `tryEnrichTableFragment` напрямую не тестируется — нужен браузерный `Range` API, linkedom не поддерживает
...
### `expandRangeToWordBoundaries`: использовать `/[\p{L}\p{N}]/u`, не `/\S/`

`/\S/` захватывает пунктуацию (`, . ; ! ? ( ) [ ]` и т.д.) — выделение расширяется до запятых и точек.
**Решение:** `/[\p{L}\p{N}]/u` — только буквы и цифры (включая Unicode/кириллицу). Флаг `u` обязателен.

### `git commit -m "$(cat <<'EOF'...)"` падает с «command not found: bat»

В `.zshrc` алиас `cat` → `bat --paging=never`. При выполнении через Bash-tool shell-алиасы не применяются, зато `$(cat <<'EOF'...)` в git hook-окружении ищет `bat` и не находит.
**Решение:** передавать сообщение коммита напрямую через `git commit -m "..."` без heredoc.
